...
class GeminiClient:
    ...
    def _trace_generation(self, trace_name: str, model: str, prompt: str,
                          ...
                          agent_name: str = "enflomnia"):
        """Log a generation event to Langfuse."""
        if not self._langfuse:
            return
        try:
            trace = self._langfuse.trace(name=trace_name, metadata={"agent": agent_name})
            trace.generation(
    ...
    def invoke_model(self, prompt: str, system: Optional[str] = None,
                     max_tokens: int = 1024, agent_name: str = "enflomnia") -> str:
        ...
        try:
            ...
            self._trace_generation("invoke_model", self.model_id, prompt, system,
                                   completion, duration_ms, agent_name)
            return completion
        except Exception as e:
    ...
    def invoke_model_json(self, prompt: str, system: Optional[str] = None,
        ...
        try:
            ...
            self._trace_generation("invoke_model_json", self.model_id, prompt, system,
                                   raw[:2000], duration_ms, agent_name)
    ...
    def generate_image(self, prompt: str, aspect_ratio: str = "1:1", agent_name: str = "image_studio") -> str:
        """Invoke the configured Gemini Image model and return a base64 string."""
        start = time.time()
        try:
