...
// Lazy-load the Player to avoid SSR issues (Remotion is browser-only)
const RemotionPlayer = dynamic(
    () => import("@remotion/player").then((mod) => mod.Player),
    { ssr: false }
);

// Import compositions (client-side only)
const ScriptVideoLazy = dynamic(
    () => import("@/remotion/compositions/ScriptVideo").then((mod) => {
        // Return a wrapper component that forwards props
        const Comp = (props: any) => <mod.ScriptVideo {...props} />;
        Comp.displayName = "ScriptVideo";
        return { default: Comp };
    }),
    { ssr: false }
);
...
function VideoPreview({ result }: { result: any }) {
    ...
    const totalFrames = useMemo(() => {
        ...
        const bodyTotal = (result.structure || []).reduce(
            (acc: number, s: any) => acc + (s.duration_seconds || DEFAULT_BODY_S),
            0
        );
        return (HOOK_S + bodyTotal + CTA_S) * 30;
    }, [result.structure]);

    if (!showPlayer) {
        return (
            <div className="card bg-gradient-to-br from-gray-900 via-gray-950 to-black border-gray-800 shadow-lg overflow-hidden">
                ...
                <button
                    ...
                    className="w-full aspect-[9/16] max-h-[500px] rounded-2xl bg-gradient-to-br from-gray-800/50 to-gray-900/50 border border-gray-800 flex flex-col items-center justify-center gap-4 hover:border-violet-500/50 transition-all group cursor-pointer"
                >
                    <div className="w-16 h-16 rounded-full bg-violet-500/20 border-2 border-violet-500/40 flex items-center justify-center group-hover:bg-violet-500/30 group-hover:border-violet-400 group-hover:scale-110 transition-all">
                        <Play size={24} className="text-violet-400 ml-1" />
                    </div>
                    <div className="text-center">
                        <p className="text-sm font-bold text-white/80">Preview Video</p>
                        <p className="text-[11px] text-gray-500 mt-1">
                            Click to render your script as an animated video
                        </p>
                    </div>
                </button>
            </div>
    ...
    return (
        <div className="card bg-gradient-to-br from-gray-900 via-gray-950 to-black border-gray-800 shadow-lg overflow-hidden">
            ...
            <div className="rounded-2xl overflow-hidden border border-gray-800">
                <RemotionPlayer
                    component={ScriptVideoLazy as any}
                    inputProps={{
                        hook: result.hook || "Your Story Starts Here",
                        structure: result.structure || [],
                        cta: result.cta || "Follow for more!",
                        tone: result.tone || "entertaining",
                    }}
            ...
            <div className="flex items-center gap-3 mt-3 px-1">
                <span className="text-[10px] font-bold text-gray-600">
                    {Math.round(totalFrames / 30)}s
                </span>
                <span className="text-gray-800">•</span>
                <span className="text-[10px] font-bold text-gray-600">1080×1920</span>
                <span className="text-gray-800">•</span>
                <span className="text-[10px] font-bold text-gray-600">30fps</span>
                <span className="text-gray-800">•</span>
                <span className="text-[10px] font-bold text-violet-500/70">
                    {(result.structure || []).length + 2} scenes
                </span>
            </div>
        </div>
    );
        ...
        <div className="p-8 max-w-5xl mx-auto space-y-8">
            ...
            <div className="flex items-start justify-between">
                ...
                <button onClick={() => handleAutoGenerate()} disabled={loading}
                    ...
                    {loading ? "Extracting insights..." : "Generate Narrative Loop"}
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {/* Remotion Video Preview — below the brief on left */}
